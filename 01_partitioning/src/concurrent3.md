...
#define SAFETY_MARGIN 2.0  // increase buffer size
...
int concurrent_output(
    ...
    size_t tuples_per_partition = (size_t)((n_tuples / n_partitions) * SAFETY_MARGIN + 1024);
    ...
    for (size_t i = 0; i < n_threads; i++) {
        ...
        thread_args[i].max_partition_size = tuples_per_partition;
        ...
        if (pthread_create(&threads[i], NULL, concurrent_thread_function, &thread_args[i]) != 0) {
            ...
            free(thread_args);
            free(threads);
            for (int j = 0; j < n_partitions; j++) free(partitions[j]);
            free(partitions);
            free(partitions_sizes);
            return EXIT_FAILURE;
        }
    ...
    *elapsed_time_ms = end_timer(start_time);
    ...
    free(thread_args);
    for (int i = 0; i < n_partitions; i++) free(partitions[i]);
    free(partitions);
    free(partitions_sizes);
    free(threads);
...
void* concurrent_thread_function(void* args) {
    ...
    for (size_t i = c_thread->start_index; i < c_thread->end_index; i++) {
        ...
        uint64_t partition_index = mod_hashing(tuple.key, c_thread->n_hash_bits);
        size_t curr_partition_size = atomic_fetch_add(&c_thread->partition_sizes[partition_index], 1);

        if (curr_partition_size >= c_thread->max_partition_size) {
            fprintf(stderr, "[Thread %d] Overflow in partition %lu\n", c_thread->thread_id, partition_index);
            continue;
        }
